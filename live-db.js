...
(function(){
  ...
  async function ensureAuth(){
    ...
    try{
      ...
      status('offline','Shared database unavailable — check Supabase connection');
      return false;
    }
  }
  async function pull(){
    ...
    }catch(e){console.warn('3FS live pull failed',e);status('offline','Local mode — database read unavailable');}
    return false;
  }
  ...
  window._3fsPushLive=function(obj){
    ...
    writeChain=writeChain.then(async()=>{
      ...
      try{
        ...
        status('offline','Local save only — shared database unavailable');
        return false;
      }
    });
    return writeChain;
